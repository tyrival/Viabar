...
删除任务、子任务和备忘录时，不得绕过现有 `TrashService` 语义。

现有 macOS `TrashService` 尚不支持项目快照。iOS 项目删除在本阶段继续对齐 macOS：保留二级确认，确认后由 `ProjectService.deleteProject(_:)` 直接删除。项目进入回收站需要扩展回收站 payload 和恢复策略，留到下一阶段单独设计。
